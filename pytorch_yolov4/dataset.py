...
class Yolo_dataset(Dataset):
    def __init__(self, lable_path, cfg, train=True):
        ...
        self.cfg = cfg
        self.train = train
        # lable_path = './data/dataset/train/train.txt'
        truth = {}
        print('label path : ',lable_path)
...
def get_image_id(filename:str) -> int:
    ...
    >>> filename = os.path.splitext(os.path.basename(filename))[0].split(".")
    >>> lv, no = os.path.splitext(os.path.basename(filename))[0].split("_")
    >>> lv = lv.replace("level", "")
    >>> no = f"{int(no):04d}"
    >>> return int(lv+no)
    """
    ## 이미지 파일 이름을 숫자로 바꿔 줘야함...
    ## apple_123.jpg, orange_123.jpg
    ## -> 1123,  3123 이런식으로 유니크한 숫자를 넘겨주도록.
...
if __name__ == "__main__":
    ...
    random.seed(2020)
    np.random.seed(2020)
    Cfg.dataset_dir = './data/dataset/train/'
